...
function loadLocal() {
    try {
        ...
        let filtered = data.kelompok.filter(
...
function genId(prefix = "") {
    return (
        ...
        Math.floor(Math.random() * 900).toString(36)
    );
}
...
function renderDashboard() {
    ...
    totalBelumNasabahEl.innerText = totals.totalBelumNas;
    totalSudahNasabahEl.innerText = totals.totalSudahNas;
...
function renderGroups() {
    ...
    if (state.kelompok.length === 0) {
        groupsListEl.innerHTML = `<div class="empty">Tidak ada penagihan hari ini. Klik "Tambah Kelompok" untuk mulai.</div>`;
        return;
    }
